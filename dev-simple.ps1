        ...
        foreach ($pid in $processes) {
            if ($pid -and $pid -ne "0") {
                try {
                    ...
                    Write-Host "   [WARN] Could not kill process $pid" -ForegroundColor Yellow
                }
            }
        }
    }
...
function Start-DevServers {
    ...
    # Start API Server
    Write-Host "   [API] Starting API Server on port 3000..." -ForegroundColor Blue
    ...
    Write-Host "   [WEB] Starting Frontend Server on port 3001..." -ForegroundColor Blue
    ...
    Write-Host "   [AI] Starting Embedding Service on port 8000..." -ForegroundColor Blue
...
function Show-Usage {
    ...
    Write-Host "   .\dev-simple.ps1 start       - Start all development servers"
    Write-Host "   .\dev-simple.ps1 stop        - Stop all development servers"
    Write-Host "   .\dev-simple.ps1 restart     - Restart all development servers"
    Write-Host "   .\dev-simple.ps1 status      - Check server status"
    Write-Host "   .\dev-simple.ps1 logs        - Show all logs"
    Write-Host "   .\dev-simple.ps1 jobs        - Show PowerShell job status"
    Write-Host ""
...
switch ($Action.ToLower()) {
    ...
    "stop" {
        ...
        Get-Job | Stop-Job
        Get-Job | Remove-Job
        Write-Host "[OK] All development servers stopped" -ForegroundColor Green
    }
    "restart" {
        Stop-DevServers
        Get-Job | Stop-Job
